	...
	public function index(){
		...
		foreach($orders as $order) {
			...
			foreach($order['OrderDetail'] as $item) {
				$portion_id = (int) $item['item_id'];

				foreach($portions[$portion_id - 1]['PortionDetail'] as $ingredient) {
					if(isset($ingredients[$ingredient['Part']['name']])) {
						$ingredients[$ingredient['Part']['name']] += (int) $ingredient['value'] * (int) $item['quantity'];
					} else {
						$ingredients[$ingredient['Part']['name']] = (int) $ingredient['value'] * (int) $item['quantity'];
					}
				}
			...
			$order_reports[$order_name] = $ingredients;
		}
